...
class MoCEIR(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        if sum(self.num_dec_blocks) > 0:
            self.total_loss /= sum(self.num_dec_blocks)
        return x


class MoceIrBaseline(BaseBaseline):
    """Wrapper moce_ir, implémentant l'interface BaseBaseline pour l'inférence."""

    def __init__(self, weights_path: str = None, **kwargs):
        super().__init__(weights_path)
        self.model = MoCEIR(**kwargs)
        if weights_path:
